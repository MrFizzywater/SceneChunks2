...
async function startServer() {
  ...
  app.post("/api/analyze-script", async (req, res) => {
    try {
      ...
      if (!scriptContent) return res.status(400).json({ error: "Script content is required" });
...
Return ONLY valid JSON matching this exact schema:
{
  ...
  "productionElements": [{ "category": "crew" | "prop" | "location" | "music" | "sfx" | "vfx", "name": "string", "description": "string", "sceneHeading": "string" }],
  "scenes": [{
...
Script Content:\n---\n${scriptContent}\n---`;
      ...
      if (!jsonText) throw new Error("Failed to generate JSON from script");
